...
class TensorHolographyModel():
    ...
    def _preprocess_input(self, example):
        # stav edit "img_%d" % i -> "img"
        # stav edit "depth_%d" % i -> "depth"
        # img_list = [example["img_%d" % i] for i in range(self.ddpm_params["active_max_ldi_layer"]+1)]
        img_list = [example["img"] for i in range(self.project_params["active_max_ldi_layer"]+1)]
        # depth_list = [example["depth_%d" % i] for i in range(self.ddpm_params["active_max_ldi_layer"]+1)]
        depth_list = [example["depth"] for i in range(self.project_params["active_max_ldi_layer"]+1)]
    ...
    def _get_loss(self,
        ...
        if y_out_phs_shifted is not None:
            ...
            pad = self.project_params["padding"]
    ...
    def validate_stage_1(self):
        ...
        # stav edit folders inside validation to be amp_4 -> amp and phs_4 -> phs
    ...
    def evaluate(self, eval_params):
        ...
        if self.project_params["activate_project"]:
            with tf.compat.v1.variable_scope("ddpm"):
                # build and run ddpm net, phs_out_shift_altered is 0 to 1
                self.model_vars_ddpm = self._build_model_vars_project(with_postfix=True)
                holo_out_shift_altered, amp, phs = self._build_graph_project(amp_phs_out_shift, self.model_vars_ddpm, with_postfix=True)
        else:
            # double phase encoding
        ...
        if self.project_params["active_max_ldi_layer"] == 0:
            # load rgbd
            ...
            for i in range(self.project_params["active_max_ldi_layer"]+1):
    ...
    def _build_model_vars_project(self, with_postfix=False):
        postfix = "_ddpm" if with_postfix else ""
        fw = np.full((self.model_params["num_layers" + postfix]), self.model_params["filter_width" + postfix],
                     dtype=int)
        fnum = np.append(np.full((self.model_params["num_layers" + postfix] - 1),
                                 self.model_params["num_filters_per_layer" + postfix], dtype=int),
                         self.model_params["output_dim" + postfix] * (
                                     self.model_params["interleave_rate" + postfix] ** 2))
        model_vars = {}

        for i in range(self.model_params["num_layers" + postfix]):
            # first layer
            if i == 0:
                in_dim, out_dim = self.model_params["input_dim" + postfix] * (
                            self.model_params["interleave_rate" + postfix] ** 2), fnum[i]
            # last layer
            elif i == self.model_params["num_layers" + postfix] - 1:
                in_dim, out_dim = fnum[i - 1], \
                                  self.model_params["output_dim" + postfix] * (
                                              self.model_params["interleave_rate" + postfix] ** 2)
                # in_dim, out_dim = fnum[i - 1] + self.model_params["input_dim" + postfix] * (
                #             self.model_params["interleave_rate" + postfix] ** 2), \
                #                   self.model_params["output_dim" + postfix] * (
                #                               self.model_params["interleave_rate" + postfix] ** 2)
            else:
            ...
            model_vars[i] = {'weights': tf_init_weights([fw[i], fw[i], in_dim, out_dim],
                                                        ...
                                                        xavier_params=(in_dim, out_dim),
                                                        r=self.model_params["weight_var_scale" + postfix]),
                             'bias': tf.Variable(tf.random.truncated_normal([out_dim], stddev=self.model_params[
                                 "bias_stddev" + postfix]))
                             }
    ...
    def _build_graph_project(self, x_in, model_vars, data_format='NCHW', with_postfix=False):
        postfix = "_ddpm" if with_postfix else ""
        layers = {}
        prev_layers = {}
        ...
        for i in range(self.model_params["num_layers" + postfix]):
            ...
            if not i == self.model_params["num_layers" + postfix] - 1:
                ...
                # compute complex field, the phase is renormalized to [-pi, pi]
                if data_format == 'NCHW':
                    amp = tf.add(field[:, :self.model_params["output_dim" + postfix] // 2, :, :] * np.sqrt(0.5),
                                 np.sqrt(0.5), name="output_field_amp" + postfix)
                    phs = tf.add(field[:, self.model_params["output_dim" + postfix] // 2:, :, :] * 0.5, 0.5,
                                 name="output_field_phs" + postfix)
                else:
    ...
    def _setup_train_project(self, holo_in, amp_in, phs_in, holo_out, amp_out, phs_out, propagator):
        pad = self.ddpm_params["padding"]
        if pad > 0:
            amp_in = tf.pad(amp_in, [[0, 0], [0, 0], [pad, pad], [pad, pad]], mode='CONSTANT', constant_values=0.0)
            phs_in = tf.pad(phs_in, [[0, 0], [0, 0], [pad, pad], [pad, pad]], mode='CONSTANT', constant_values=0.5)
        ...
        with tf.compat.v1.variable_scope("ddpm"):
            ...
            self.model_vars_ddpm = self._build_model_vars_project(with_postfix=True)
            holo_out_shift_altered, amp_out_shift_altered, phs_out_shift_altered = \
                self._build_graph_project(amp_phs_out_shift, self.model_vars_ddpm, with_postfix=True)
    ...
    def project(self):
        ...
        holo_out_s2, amp_out_s2, phs_out_s2, \
        amp_out_shifted_altered_s2, phs_out_shifted_altered_s2, \
        amp_out_shifted_s2, phs_out_shifted_s2 = self._setup_train_project(
            holo_in, amp_in, phs_in, holo_out, amp_out, phs_out, propagator_pad
        )
        ...
        loss_identity = self.loss_params["loss_op"](amp_out_shifted_s2, amp_out_shifted_altered_s2) + \
                        self.loss_params["loss_op"](phs_out_shifted_s2, phs_out_shifted_altered_s2)
        ssim_amp_loss_identity = tf.reduce_mean(tf.image.ssim(tf.transpose(amp_out_shifted_s2, [0, 2, 3, 1]),
                                                              tf.transpose(amp_out_shifted_altered_s2, [0, 2, 3, 1]),
                                                              1.0))
        ...
        # stage 2 identity pre-training
        train_identity_op = optimizer.minimize(loss=loss_identity, global_step=global_step, var_list=ddpm_vars) if not \
        self.ddpm_params["bypass_ddpm_network"] else None
        ...
        for epoch in range(last_epoch, self.training_params["num_epochs"]):
            ...
            for t_step in range(train_steps):
                ...
                # if epoch < self.training_params["epoch_to_start_ddpm_training"] + 50 and \
                #         self.project_params["activate_project"]:
                #     # stage 2 pre-training identity mapping
                #     training_flag = "stage 2 pre-training identity mapping\n"
                #     _, loss_val, ssim_amp_val = self.sess.run([train_identity_op,
                #                                                loss_identity,
                ...
                #     print("Epoch %d, Step %d, total_loss %0.8f, ssim amp loss %0.8f\n" % \
                #           (epoch, t_step, loss_val, ssim_amp_val))
                elif self.project_params["activate_project"]:
                    # stage 2 training with main cnn and ddpm cnn
                    training_flag = "stage 2 training, both main cnn and ddpm cnn\n"
                    _, loss_val, ssim_amp_val, mean_loss_val, std_loss_val = self.sess.run([train_ddpm_full_op,
                                                                                            loss_s2,
...
if __name__ == '__main__':
    ...
    # stav change data -> ../../../databases/lotanstav@staff.technion.ac.il
    parser.add_argument('--eval-rgb-path', default=os.path.join(cur_dir, "../../../databases/lotanstav@staff.technion.ac.il", "example_input", "bbb_rgb.png"), help='Input rgb image path in evaluation mode')
    parser.add_argument('--eval-depth-path', default=os.path.join(cur_dir, "../../../databases/lotanstav@staff.technion.ac.il", "example_input", "bbb_depth.png"), help='Input depth image path in evaluation mode')
    parser.add_argument('--eval-output-path', default=os.path.join(cur_dir, "../../../databases/lotanstav@staff.technion.ac.il", "example_input"), help='Output directory for results')
  ...
  # project parameters
    parser.add_argument('--project-mode', action='store_true', help='Run in project mode')
    parser.add_argument('--activate-project', action='store_true', help='Load ddpm network together with hologram rendering network; depth shift specified by --train-depth-shift')
    ...
    parser.add_argument('--epoch_to_start_ddpm_training', default=1, type=int, help='The epoch to start stage-2 training')
    ...
    project_params = {
        "active_max_ldi_layer": opt.active_max_ldi_layer,          # active maximum ldi layer index, default to 0
        "activate_project": opt.activate_project,                        # activate project
        "bypass_project_network": opt.bypass_ddpm_network,            # bypass project network
        "padding": opt.padding,                                    # padding to accommodate out-of-frame diffraction
    }
    ...
    model_params = {
        ...
        "num_layers_ddpm":2,                                       # number of ddpm network layers
    ...
    # path parameters
    # stav change "amp_4", "phs_4" -> "amp", "phs"
    # labels = ["amp_4", "phs_4"]
    labels = ["amp", "phs"]
    for i in range(opt.active_max_ldi_layer+1):
        # stav change "img_" + str(i) -> "img"
        # stav change "depth_" + str(i) -> "depth"
    ...
    # stav change data -> ../../../databases/lotanstav\@staff.technion.ac.il
    train_base_path = os.path.join("/../../../databases/lotanstav@staff.technion.ac.il/train_%d_v2" % opt.dataset_res)
    test_base_path = os.path.join(cur_dir, "../../../databases/lotanstav@staff.technion.ac.il", "test_%d_v2" % opt.dataset_res)
    validate_base_path = os.path.join(cur_dir, "../../../databases/lotanstav@staff.technion.ac.il", "validate_%d_v2" % opt.dataset_res)
